        ...
        socket.on('identification', function(config, cb) {
            Device.setOnline(config, function(device){
                ...
                if (device.user === undefined) {
                    cb(false);
                    device.onSaveOnce(function(doc){
                        if (doc.user !== undefined) {
                            User.findOne({_id:doc.user}).exec(function(err, user){
                                if (err) {
                                    console.error(err);
                                    return;
                                }
                                socket.emit("found_user", user);
                            });
                            return true;
                        }
                        return false;
                    ...
                    cb(true);
                    User.findOne({_id:device.user}).exec(function(err, user){
                        if (err) {
                            console.error(err);
                            return;
                        }
